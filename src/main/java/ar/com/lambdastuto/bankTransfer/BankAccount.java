...
public class BankAccount {
    ...
    public boolean withdraw(double  ammount) throws InterruptedException {
        if(this.lock.tryLock()){
            Thread.sleep(100);
            balance = subtractFunction.apply(balance, ammount);
            this.lock.unlock();
            return true;
        }
        return false;
    ...
    public boolean transfer(BankAccount bankAccount, double amount) throws InterruptedException {
        if(withdraw(amount)){
            System.out.println("Withdrawing amount: "+ amount + " from: "+ getAccountName());
            if(bankAccount.deposit(amount)){
                System.out.println("Depositing amount: "+ amount+ " to: " + bankAccount.getAccountName());
                return true;
            } else {
